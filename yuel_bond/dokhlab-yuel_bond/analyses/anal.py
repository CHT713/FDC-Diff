...
groups = {
    'geom_3d': ['geom_test', 'geom_test_predictions_3d'],
    'geom_2d': ['geom_test_bonds', 'geom_test_predictions_2d'],
    'geom_kekulized_3d': ['geom_kekulized_test', 'geom_kekulized_test_predictions_3d'],
    'geom_kekulized_noise_0_2_3d': ['geom_kekulized_test_noise_0_2', 'geom_kekulized_test_noise_0_2_predictions_3d'],
    'geom_kekulized_2d': ['geom_kekulized_test_bonds', 'geom_kekulized_test_predictions_2d'],
    # 'geom_sanitized_3d': ['geom_sanitized_test_3d', 'geom_sanitized_test_predictions_3d'],
    'geom_sanitized_noise_0_2_3d': ['geom_sanitized_test_noise_0_2', 'geom_sanitized_test_noise_0_2_predictions_3d'],
    # 'geom_sanitized_2d': ['geom_sanitized_test_2d', 'geom_sanitized_test_predictions_2d'],
}

def load_data(group):
    ground_truth = torch.load(f'../datasets/{group[0]}.pt', map_location=torch.device('cpu'))
    predictions = torch.load(f'../tests/{group[1]}.pt', map_location=torch.device('cpu'))
...
def plot_bond_type_metrics(json_data, outprefix=''):
    for bond_type, metrics in json_data.items():
        bond_type = int(bond_type)
        if bond_type < 4:
            plot_metrics(metrics['accuracy'], metrics['precision'], metrics['recall'], metrics['f1'], const.BOND_TYPE_NAMES[bond_type], outfile=f'figures/{outprefix}_bond_type_{bond_type}.svg')
...
def print_metrics(metrics):
    for item in ['accuracy', 'precision', 'recall', 'f1']:
        if item in metrics:
            print(f'{item}: {np.mean(metrics[item])}')

#%%
# if __name__ == '__main__':
#     metrics = {name: calculate_metrics(*load_data(group)) for name, group in groups.items()}
#     for name, metric in metrics.items():
#         save_metrics(metric, f'metrics/{name}.json')
#         print(name)
#         plot_metrics(metric['accuracy'], metric['precision'], metric['recall'], metric['f1'], outfile=f'figures/{name}_all.svg')
...
#         print(name)
#         plot_bond_type_metrics(metric, outprefix=f'{name}')
#         save_metrics(metric, f'metrics/{name}_per_bond_type.json')

#%%
if __name__ == '__main__':
    for name, group in groups.items():
        print(name)
        metrics_file = f'metrics/{name}.json'
        metrics_per_bond_type_file = f'metrics/{name}_per_bond_type.json'
        if not os.path.exists(metrics_file):
            metric = calculate_metrics(*load_data(group))
            save_metrics(metric, metrics_file)
        if not os.path.exists(metrics_per_bond_type_file):
            metric = calculate_per_bond_type_metrics(*load_data(group))
            save_metrics(metric, metrics_per_bond_type_file)
        metrics = load_metrics(metrics_file)
        metrics_per_bond_type = load_metrics(metrics_per_bond_type_file)

        plot_metrics(metrics['accuracy'], metrics['precision'], metrics['recall'], metrics['f1'], outfile=f'figures/{name}_all.svg')
        plot_bond_type_metrics(metrics_per_bond_type, outprefix=f'{name}')
        # print metrics and metrics per bond type together like a table
        # print both average and standard deviation

        print(f"{'Metric':<15}{'All':<15}{'Bond Type 0':<15}{'Bond Type 1':<15}{'Bond Type 2':<15}{'Bond Type 3':<15}")
        print('-' * 60)
        print(f"{name:<15}{np.mean(metrics['accuracy']):<15.4f}{np.std(metrics['accuracy']):<15.4f}{np.mean(metrics['precision']):<15.4f}{np.std(metrics['precision']):<15.4f}{np.mean(metrics['recall']):<15.4f}{np.std(metrics['recall']):<15.4f}{np.mean(metrics['f1']):<15.4f}{np.std(metrics['f1']):<15.4f}")
        for name, metric in metrics_per_bond_type.items():
            name = int(name)
            if name < 4:
                print(f"{name:<15}{np.mean(metric['accuracy']):<15.4f}{np.std(metric['accuracy']):<15.4f}{np.mean(metric['precision']):<15.4f}{np.std(metric['precision']):<15.4f}{np.mean(metric['recall']):<15.4f}{np.std(metric['recall']):<15.4f}{np.mean(metric['f1']):<15.4f}{np.std(metric['f1']):<15.4f}")
