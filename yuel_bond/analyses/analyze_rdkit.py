...
def plot_metrics(metrics1, metrics2, title='', outfile=''):
    ...
    accuracies1 = metrics1['accuracy']
    precisions1 = metrics1['precision']
    recalls1 = metrics1['recall']
    f1_scores1 = metrics1['f1']

    accuracies2 = metrics2['accuracy']
    precisions2 = metrics2['precision']
    recalls2 = metrics2['recall']
    f1_scores2 = metrics2['f1']
...
rdkit_metrics = evaluate_sdf_bonds(
    '../tests/geom_sanitized_test_noise_0_2_rdkit_original.sdf',
    '../tests/geom_sanitized_test_noise_0_2_rdkit_rebuilt.sdf',
    '../analyses/figures/rdkit_metrics_noise_0_2.svg')
yuel_metrics = evaluate_sdf_bonds(
    '../tests/geom_sanitized_test_noise_0_2_true_3d.sdf',
    '../tests/geom_sanitized_test_noise_0_2_predictions_3d.sdf',
    '../analyses/figures/yuel_metrics_sanitized_noise_0_2.svg')

#%%

save_metrics(rdkit_metrics, '../analyses/metrics/rdkit_metrics_noise_0_2.json')
save_metrics(yuel_metrics, '../analyses/metrics/yuel_metrics_sanitized_noise_0_2.json')

#%%
# plot_metrics(rdkit_metrics, yuel_metrics, title='Rdkit vs Yuel', outfile='analyses/figures/rdkit_vs_yuel_noise_0_2.svg')
plot_metrics(yuel_metrics, rdkit_metrics, outfile='../analyses/figures/yuel_vs_rdkit_sanitized_noise_0_2.svg')

# %%

# load the metrics and print like a table
with open('../analyses/metrics/rdkit_metrics_noise_0_2.json', 'r') as f:
    rdkit_metrics = json.load(f)
    if 'accuracies' in rdkit_metrics:
        rdkit_metrics['accuracy'] = rdkit_metrics['accuracies']
    if 'precisions' in rdkit_metrics:
        rdkit_metrics['precision'] = rdkit_metrics['precisions']
    if 'recalls' in rdkit_metrics:
        rdkit_metrics['recall'] = rdkit_metrics['recalls']
    if 'f1_scores' in rdkit_metrics:
        rdkit_metrics['f1'] = rdkit_metrics['f1_scores']

# with open('../analyses/metrics/yuel_metrics_sanitized_noise_0_2.json', 'r') as f:
#     yuel_metrics = json.load(f)
with open('../analyses/metrics/geom_sanitized_noise_0_2_3d.json', 'r') as f:
    yuel_metrics = json.load(f)

#%%

# print the metrics in a table with means and standard deviations
print(f"{'Metric':<15}{'Rdkit Mean':<15}{'Rdkit Std':<15}{'Yuel Mean':<15}{'Yuel Std':<15}")
print('-' * 60)
for metric in ['accuracy', 'precision', 'recall', 'f1']:
    print(f"{metric:<15}"
          f"{np.mean(rdkit_metrics[metric]):<15.4f}"
          f"{np.std(rdkit_metrics[metric]):<15.4f}"
          f"{np.mean(yuel_metrics[metric]):<15.4f}"
          f"{np.std(yuel_metrics[metric]):<15.4f}")
