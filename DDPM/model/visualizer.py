...
from model.molecule_builder import *
...
def save_xyz_file_fa_temp(path, one_hot, positions, node_mask, names, suffix=''):
    """
    临时版本：直接使用 positions 的第3维作为原子数，忽略 node_mask。

    Args:
        path (str): 输出文件路径
        one_hot (torch.Tensor): 原子类型的 one-hot 编码，形状 [batch_size, n_nodes, n_features]
        positions (torch.Tensor): 原子坐标，形状 [batch_size, n_nodes, 3] 或 [batch_size, frames, n_nodes, 3]
        node_mask (torch.Tensor): 原子掩码（此函数中未使用，仅占位）
        names (list): 文件名列表
        suffix (str): 文件名后缀
    """
    idx2atom = const.IDX2ATOM
    ...
    for batch_i in range(one_hot.size(0)):
        # 直接从 positions 的第3维获取原子数
        if positions.dim() == 4:  # 例如 [1, 1, 16, 3]
            n_atoms = positions.size(2)  # 16
            atom_idx = torch.arange(n_atoms, device=positions.device)  # [0, 1, ..., 15]
            pos = positions[batch_i, 0]  # [16, 3]
        elif positions.dim() == 3:  # 例如 [1, 16, 3]
            n_atoms = positions.size(1)  # 16
            atom_idx = torch.arange(n_atoms, device=positions.device)  # [0, 1, ..., 15]
            pos = positions[batch_i]  # [16, 3]
        else:
        ...
        # 从 one_hot 获取原子类型
        atoms = torch.argmax(one_hot[batch_i], dim=1)  # [n_nodes]
