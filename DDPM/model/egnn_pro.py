...
def setup_logging(log_file='egnn1_log.log'):
    logging.basicConfig(
        filename=log_file,
        level=logging.INFO,
...
class EquivariantBlock(nn.Module):
    ...
    def __init__(self, hidden_nf, edge_feat_nf=2, device='cpu', activation=nn.SiLU(), n_layers=2, attention=True,
        ...
        self.add_module("gcl_equiv",
                        EquivariantUpdate(hidden_nf, edges_in_d=edge_feat_nf, activation=activation, tanh=tanh,
...
class EGNN(nn.Module):
    ...
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', activation=nn.SiLU(), n_layers=3,
                 ...
                 sin_embedding=False, normalization_factor=100, aggregation_method='sum', task_aware=True,num_atom_types_with_hash=None):
        ...
        self.atom_type_mlp = nn.Sequential(
            nn.Linear(self.hidden_nf, self.hidden_nf),
            activation,
            nn.Linear(self.hidden_nf, num_atom_types_with_hash)  # 输出维度是原子类型的数量
        )

        # 新增: 原子有效性预测头
        self.valid_head = nn.Sequential(
            nn.Linear(self.hidden_nf, self.hidden_nf // 2),  # 可以根据需要调整维度
            activation,
            nn.Linear(self.hidden_nf // 2, 1)  # 输出一个logit用于二分类
        )
        # 注意: 坐标的预测 (x_final) 是由 EquivariantBlock 直接输出的，不需要单独的头，但其损失计算会受valid_mask影响
    ...
    def forward(self, h, x, edge_index, node_mask=None, rgroup_mask=None, edge_mask=None, task_feat=None):
        ...
        atom_type_logits = self.atom_type_mlp(h)  # [B*N, num_types_with_hash]
        valid_logits = self.valid_head(h)      # [B*N, 1]


        # Important, the bias of the last linear might be non-zero

        if node_mask is not None:
            atom_type_logits = atom_type_logits * node_mask
            valid_logits = valid_logits * node_mask
        # 每个 batch 处理完后添加分隔线
        logging.info("----------")
        return x, atom_type_logits, valid_logits  # 返回预测的坐标, 原子类型logits, 有效性logits
